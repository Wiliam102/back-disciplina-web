...
public class TaskService {
    ...
    public Task save(Task task){
        ...
        if(task == null){
           throw new RuntimeException("A descrição da tarefa não pode ser nula");
        }
        if(task.getDescription().equals("") || task.getDescription().equals(" ")){
            throw new RuntimeException("A descrição da tarefa não pode ser vazia");
        }
        // verificando se a tarefa já esta cadastrada
        if(taskRepository.existsTaskByDescription(task.getDescription())){
            throw new RuntimeException("A tarefa já esta cadastrada.");
        }
    ...
    public Task update(Task taskUpdate, Long id){
        ...
        // lançando excecao caso nao encontre
